...
def plot_postProcessing_1_OK_61(angle0=2160.0):
    df = pd.read_csv("postProcessing_1_OK_61.5m/turbines/0/turbine.csv")
    ...
    plt.ylabel("$C_P$")
    plt.title("plot_postProcessing_1_OK_61")
    plt.tight_layout()

def plot_postProcessing_1_OK_noTOWER_61(angle0=2160.0):
    df = pd.read_csv("postProcessing_1_OK_noTOWER_61.5/turbines/0/turbine.csv")
    df = df.drop_duplicates("time", keep="last")
    if df.angle_deg.max() < angle0:
        angle0 = 0.0
    print("Performance from {:.1f}--{:.1f} degrees:".format(
    ...
    plt.xlabel("Azimuthal angle (degrees)")
    plt.ylabel("$C_P$")
    plt.title("plot_postProcessing_1_OK_noTOWER_61")    
    plt.tight_layout()
    
# def plot_postProcessing_1_OK_withTower_61(angle0=2160.0):
#     df = pd.read_csv("postProcessing_1_OK_withTower_61.5/turbines/0/turbine.csv")
#     df = df.drop_duplicates("time", keep="last")
#     if df.angle_deg.max() < angle0:
#         angle0 = 0.0
#     print("Performance from {:.1f}--{:.1f} degrees:".format(
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        plot_postProcessing_1_OK_61()
        plot_postProcessing_1_OK_noTOWER_61()
        # plot_postProcessing_1_OK_withTower_61()
    plt.show()
